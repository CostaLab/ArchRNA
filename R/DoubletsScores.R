...
# Adjusted from ArchR https://github.com/GreenleafLab/ArchR
##########################################################################################
...
addDoubletScores_ <- function(
  ...
  ArchR:::.validInput(input = input, name = "input", valid = c("character", "ArchRProject"))
  ArchR:::.validInput(input = useMatrix, name = "useMatrix", valid = c("character"))
  ArchR:::.validInput(input = k, name = "k", valid = c("integer"))
  ArchR:::.validInput(input = nTrials, name = "nTrials", valid = c("integer"))
  ArchR:::.validInput(input = dimsToUse, name = "dimsToUse", valid = c("integer", "null"))
  ArchR:::.validInput(input = corCutOff, name = "corCutOff", valid = c("numeric", "null"))
  ArchR:::.validInput(input = scaleDims, name = "scaleDims", valid = c("boolean"))
  ArchR:::.validInput(input = knnMethod, name = "knnMethod", valid = c("character"))
  ArchR:::.validInput(input = UMAPParams, name = "UMAPParams", valid = c("list"))
  ArchR:::.validInput(input = LSIParams, name = "LSIParams", valid = c("list"))
  ...
  ArchR:::.logThis(mget(names(formals()),sys.frame(sys.nframe())), "addDoubletScores Input-Parameters", logFile = logFile)

  if(tolower(useMatrix) %ni% c("peakmatrix","tilematrix", "geneexpressionmatrix")){
    stop(sprintf("Supported Matrix Names at the moment are PeakMatrix,  TileMatrix and GeneExpressionMatrix : ", useMatrix))
  }
  ...
  outList <- ArchR:::.batchlapply(args, sequential = TRUE)
...
.addDoubScores_ <- function(
  ...
  ArchR:::.logDiffTime(sprintf("%s Computing Doublet Statistics", prefix), tstart, addHeader = FALSE, verbose = verbose, logFile = logFile)
  ...
  if(is.null(allCells)){
    proj@cellColData <- proj@cellColData[ArchR:::.availableCells(ArrowFile, useMatrix),]
  }else{
  ...
  #################################################
  ArchR:::.logDiffTime("Running IterativeLSI", tstart, addHeader = FALSE, verbose = FALSE, logFile = logFile)
  ...
  proj <- tryCatch({
    do.call(addRNAIterativeLSI, LSIParams)
  }, error = function(e){
    ArchR:::.logError(e, fn = "addRNAIterativeLSI", info = prefix, errorList = list(ArrowFile = ArrowFile), logFile = logFile)
  })

  #################################################
  # 3. Get LSI Partial Matrix For Simulation
  #################################################
  ArchR:::.logDiffTime("Constructing Partial Matrix for Projection", tstart, addHeader = FALSE, verbose = FALSE, logFile = logFile)
  LSI <- getReducedDims(
    ArchRProj = proj,
    reducedDims = "RNAIterativeLSI",
    corCutOff = corCutOff,
    dimsToUse = dimsToUse,
    scaleDims = scaleDims,
    returnMatrix = FALSE
  )
  ArchR:::.logThis(LSI, name = paste0(prefix, "LSI Result"), logFile = logFile)

  LSIDims <- seq_len(ncol(LSI[[1]]))
  if(length(LSIDims) < 2){
    ArchR:::.logMessage("Reduced LSI Dims below 2 dimensions, please increase dimsToUse or increase corCutOff!")
    stop("Reduced LSI Dims below 2 dimensions, please increase dimsToUse or increase corCutOff!")
  }
  featureDF <- LSI$LSIFeatures

  mat <- tryCatch({
    ArchR:::.getPartialMatrix(
    ...
    ArchR:::.logError(e, fn = "getPartialMatrix", info = prefix, errorList = errorList, logFile = logFile)
  })
  cellNames <- rownames(getCellColData(proj))
  ...
  #################################################
  ArchR:::.logDiffTime("Running LSI UMAP", tstart, addHeader = FALSE, verbose = FALSE, logFile = logFile)
  set.seed(1) # Always do this prior to UMAP
  UMAPParams <- ArchR:::.mergeParams(UMAPParams, list(n_neighbors = 40, min_dist = 0.4, metric="euclidean", verbose=FALSE))
  UMAPParams$X <- LSI$matSVD
  UMAPParams$ret_nn <- TRUE
  UMAPParams$ret_model <- TRUE
  UMAPParams$n_threads <- subThreads
  ArchR:::.logThis(UMAPParams, name = paste0(prefix, "UMAP Params"), logFile = logFile)
  uwotUmap <- tryCatch({
  ...
  #################################################
  ArchR:::.logDiffTime("Simulating and Projecting Doublets", tstart, addHeader = FALSE, verbose = FALSE, logFile = logFile)
  simDoubletsSave <- tryCatch({
    .simulateProjectDoublets_(
      mat = mat,
      LSI = LSI,
      sampleRatio1 = c(1/2),
      sampleRatio2 = c(1/2),
    ...
    ArchR:::.logError(e, fn = ".simulateProjectDoublets", info = prefix, errorList = errorList, logFile = logFile)
  })
  ...
  ArchR:::.logThis(simDoublets, name = paste0(prefix, "SimulationResults"), logFile = logFile)
  ...
  ##################################
  #Save Results
  ArchR:::.logThis(df, name = paste0(prefix, "Sample UMAP"), logFile = logFile)
  ArchR:::.logThis(dfDoub, name = paste0(prefix, "Simulated Doublet UMAP"), logFile = logFile)
  ...
  ArchR:::.safeSaveRDS(summaryList, file.path(outDir, paste0(ArchR:::.sampleName(ArrowFile), "-Doublet-Summary.rds")))
  rm(simDoubletsSave)
  ##################################
  ...
  o <- tryCatch({
    ...
    pdf(file.path(outDir, paste0(ArchR:::.sampleName(ArrowFile), "-Doublet-Summary.pdf")), width = 6, height = 6)
    ...
    pdensity <- ggplot() +
      ArchR:::.geom_point_rast2(data = df, aes(x=X1,y=X2),color="lightgrey", size = 0.5) +
      ArchR:::.geom_point_rast2(data = dfDoub, aes(x=x,y=y,colour=color), size = 0.5) +
    ...
    ArchR:::.fixPlotSize(penrich, plotWidth = 6, plotHeight = 6)
    grid::grid.newpage()

    #2. Doublet Scores
    ArchR:::.fixPlotSize(pscore, plotWidth = 6, plotHeight = 6)
    grid::grid.newpage()

    #3. Doublet Density
    ArchR:::.fixPlotSize(pdensity, plotWidth = 6, plotHeight = 6)

    dev.off()

  }, error = function(e){
    errorList <- list(df=df,dfDoub=dfDoub)
    ArchR:::.logError(e, fn = "ggplot", info = prefix, errorList = errorList, logFile = logFile, throwError = FALSE)
  })
...
.simulateProjectDoublets_ <- function(
  mat = NULL,
  LSI = NULL,
  uwotUmap = NULL,
  sampleRatio1 = c(0.5),
  ...
  errorList <- append(args, mget(names(formals()),sys.frame(sys.nframe())))
  simLSI <- tryCatch({
    ArchR:::.safelapply(seq_len(nTrials), function(y){
      if(y %% 5 == 0){
        gc()
      }
    ...
    ArchR:::.logError(e, fn = "Simulate LSI Project Doublets", info = prefix, errorList = errorList, logFile = logFile)
  })

  ArchR:::.logThis(simLSI, name = paste0(prefix, "SimulatedLSI"), logFile = logFile)

  #Compute original
  ogLSI <- suppressMessages(ArchR:::.projectLSI(mat, LSI))

  ArchR:::.logThis(ogLSI, name = paste0(prefix, "OriginalLSI"), logFile = logFile)
  ArchR:::.logThis(LSI[[1]], name = paste0(prefix, "OriginalReProjectedLSI"), logFile = logFile)
  ...
  umapProject <- tryCatch({
    ...
    ArchR:::.logError(e, fn = "uwot::umap_transform", info = prefix, errorList = errorList, logFile = logFile)
  })
  ...
  ArchR:::.logThis(uwotUmap[[1]], name = paste0(prefix, "OriginalUMAP"), logFile = logFile)
  ArchR:::.logThis(umapProject[-seq_len(nSimLSI), ], name = paste0(prefix, "OriginalReProjectedUMAP"), logFile = logFile)

  msg <- paste0(prefix, "UMAP Projection R^2 = ", round(mean(corProjection[[2]])^2, 5))
  ArchR:::.logMessage(msg, logFile = logFile)
  message(msg)
  ...
  out2 <- tryCatch({
    ...
    knnDoub <- ArchR:::.computeKNN(allLSI[-seq_len(nSimLSI),], allLSI[seq_len(nSimLSI),], k)
    ...
    ArchR:::.logThis(countKnn, paste0(prefix, "Number of Nearby Simulated Doublets"), logFile = logFile)
    ...
    ArchR:::.logThis(doubletScore, paste0(prefix, "DoubletScoresLSI"), logFile = logFile)
    ArchR:::.logThis(doubletEnrich, paste0(prefix, "DoubletEnrichLSI"), logFile = logFile)
    ...
    ##############################################################################
    knnDoub <- ArchR:::.computeKNN(umapProject[-seq_len(nSimLSI),], umapProject[seq_len(nSimLSI),], k)
    ...
    doubletEnrich <- (countKnn / sum(countKnn)) / (1 / nrow(LSI$matSVD))
    doubletEnrich <- 10000 * doubletEnrich / length(countKnn) #Enrichment Per 10000 Cells in Data Set
    ArchR:::.logThis(doubletScore, paste0(prefix, "DoubletScoresUMAP"), logFile = logFile)
    ArchR:::.logThis(doubletEnrich, paste0(prefix, "DoubletEnrichUMAP"), logFile = logFile)
...
#' @export
addDemuxletResults_ <- function(ArchRProj = NULL, bestFiles = NULL, sampleNames = NULL){

  ArchR:::.validInput(input = ArchRProj, name = "ArchRProj", valid = c("ArchRProj"))
  ArchR:::.validInput(input = bestFiles, name = "bestFiles", valid = c("character"))
  ArchR:::.validInput(input = sampleNames, name = "sampleNames", valid = c("character"))

  ArchR:::.requirePackage("readr", source = "cran")
  ...
  for(x in seq_along(bestFiles)){
    best <- ArchR:::.suppressAll(data.frame(readr::read_tsv(bestFiles[x])))
