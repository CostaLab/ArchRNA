...
addModuleScoreRNA <- function(
  ...
  if(!is.null(seqname)){
    idx <- lapply(seq_along(unlist(features)), function(x){
      ix <- intersect(which(tolower(unlist(features)[x]) == tolower(featureDF$name)), BiocGenerics::which(tolower(seqname) == tolower(featureDF$seqnames)))
      if(length(ix)==0){
        ArchR:::.logStop(sprintf("FeatureName (%s) does not exist! See getFeatures", unlist(features)[x]), logFile = logFile)
      }
      ix
    }) %>% unlist
  ...
  if(any(is.na(idx))){
    ArchR:::.logStop(sprintf("FeatureName (%s) does not exist! See getFeatures", paste0(unlist(features)[which(is.na(idx))], collapse=",")), logFile = logFile)
  }
  ...
  featuresUse  <- featureDF[idx, ,drop=FALSE]
  ...
  if(is.null(ncol(dfM))){
    ArchRProj <- addCellColData(ArchRProj, data = dfM, name=names(featureList)[1], cells=names(dfM), force = TRUE)
  }else{
