verbose_func <- function(x, verbose=FALSE){
  if(verbose){
    return(x)
  }else{
...
#' @param rowname where is rowname from, meta or rowRanges
...
getMatrix.ArchRProject <- function(
  ...
  matrix_name,
  assay="counts",
  rowname="meta", # rowRanges
  ...){
  if(!(matrix_name %in%getAvailableMatrices(object))){
    stop(paste0("matrix ", matrix_name, " is not in object"))
  }
  seMtx <- getMatrixFromProject(object, matrix_name, assay=assay)
  rnms <- NULL
  if (rowname == "meta"){
    rnms <- elementMetadata(seMtx)$name
  }else if(rowname=="rowRanges"){
    dff <- as.data.frame(rowRanges(seMtx))
    rnms <- paste(dff[["seqnames"]],dff[["start"]],dff[["end"]], sep="_")
  }else{
    stop("invalid rowname meta or rowRanges")
  }
  if(is.null(assay)){
